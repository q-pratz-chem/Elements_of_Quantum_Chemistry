...
C    ____________________CIS___________________________
C    __________________________________________________

      print *, "___________  CIS CORRECTION ______________"

C     _________CONVERT SPATIAL TO SPIN, MO BASIS 2 e-integrals    
C     _________Spin basis double bar integral (Physicists' notation)    

      allocate (spin_int(nbasis*2,nbasis*2,nbasis*2,nbasis*2))
      val_1 = 0.0d0
      val_2 = 0.0d0
      ...
      do i=1, nbasis*2
        do j=1, nbasis*2
          do k=1, nbasis*2
            do l=1, nbasis*2
      val_1 = mo_2eint((i+1)/2,(k+1)/2,(j+1)/2,(l+1)/2)
     &  *(mod(i,2).eq.mod(k,2)) *(mod(j,2).eq.mod(l,2))

      val_2 = mo_2eint((i+1)/2,(l+1)/2,(j+1)/2,(k+1)/2)
     &  *(mod(i,2).eq.mod(l,2)) *(mod(j,2).eq.mod(k,2))

           spin_int(i,j,k,l)= val_1 - val_2
            end do
          end do
        end do
      end do

C     _____Spin basis fock matrix eigenvalues

      allocate (fs(nbasis*2,nbasis*2))
      allocate (fs_vec(nbasis*2,nbasis*2))
      fs = 0.0d0
      fs_vec = 0.0d0
      do i = 1, nbasis*2
         fs(i,i) = orb_E((i+1)/2)
      ...
      Call EIG(fs, fs_vec, i, nbasis*2, 1)



c     ________Building Hamiltonian for CIS

       tmp_1 = 2*nocc*2*unocc
      allocate (H_cis(tmp_1,tmp_1))
      allocate (e_vec(tmp_1,tmp_1))
      ...
      do i = 1, nelctrn
        do a = nelctrn+1,nbasis*2
           ia = ia+1
           jb = 0
          do j = 1,nelctrn
            do b = nelctrn+1, nbasis*2
             jb = jb+1
      H_cis(ia,jb)=(fs(a,a)-fs(i,i))*(i.eq.j)*(a.eq.b)
     & + spin_int(a,j,i,b)
            end do
          end do
        end do
...
C     _____Diagonalizing CIS-HAMILTONIAN Matrix________
      Call EIG(H_cis, e_vec, i, tmp_1, 1)
